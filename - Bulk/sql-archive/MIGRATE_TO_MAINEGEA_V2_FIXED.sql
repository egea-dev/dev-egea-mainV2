...
CREATE POLICY "Los usuarios autenticados pueden ver los vehÃ­culos"
  ON public.vehicles FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Los administradores pueden gestionar los vehÃ­culos"
...
CREATE POLICY "Los usuarios autenticados pueden ver las asignaciones de vehÃ­culos"
...
DROP POLICY IF EXISTS "Cualquier usuario autenticado puede ver los vehÃ­culos" ON public.vehicles;
DROP POLICY IF EXISTS "Solo los administradores pueden gestionar vehÃ­culos" ON public.vehicles;
...
CREATE POLICY "Usuarios autenticados pueden ver los vehÃ­culos"
  ON public.vehicles FOR SELECT
  TO authenticated
  USING (true);

-- INSERT (Solo Admin): Solo los administradores pueden crear vehÃ­culos.
CREATE POLICY "Administradores pueden crear vehÃ­culos"
  ON public.vehicles FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND team = 'admin'));

-- UPDATE (Solo Admin): Solo los administradores pueden actualizar vehÃ­culos.
CREATE POLICY "Administradores pueden actualizar vehÃ­culos"
  ON public.vehicles FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND team = 'admin'));

-- DELETE (Solo Admin): Solo los administradores pueden eliminar vehÃ­culos.
CREATE POLICY "Administradores pueden eliminar vehÃ­culos"
...
DROP POLICY IF EXISTS "Los admins pueden gestionar vehÃ­culos" ON public.vehicles;
...
/*
-- Habilitar la extensiÃ³n pg_cron (solo una vez)
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Cron Job: Archivar tareas completadas todos los dÃ­as a las 02:00 AM
SELECT cron.schedule(
  'archive-completed-tasks-daily',     -- Nombre del job
  '0 2 * * *',                        -- Cron expression: 02:00 AM todos los dÃ­as
