-- ============================================================================
-- MIGRACIÓN FINAL - ELIMINA DUPLICADOS Y CORRIGE TODO
...
-- ============================================================================
-- PARTE 2: ELIMINAR ROLES DUPLICADOS EN INGLÉS (NO RENOMBRAR)
-- ============================================================================

-- Simplemente eliminar los roles en inglés si existen
-- Los roles en español ya existen y están correctos
DELETE FROM public.role_permissions WHERE role IN ('production', 'shipping', 'warehouse');
DELETE FROM public.profiles WHERE role IN ('production', 'shipping', 'warehouse');

DO $$ BEGIN
  RAISE NOTICE '✓ Paso 2/4: Roles en inglés eliminados (duplicados)';
END $$;
...
BEGIN
  ...
  SELECT string_agg(role, ', ' ORDER BY role) INTO v_perm_roles
  FROM (SELECT DISTINCT role FROM public.role_permissions) AS rp;
  
  RAISE NOTICE '================================================';
  RAISE NOTICE '✅ MIGRACIÓN COMPLETADA EXITOSAMENTE';
  RAISE NOTICE '================================================';
  RAISE NOTICE 'Roles en profiles: %', v_roles;
  RAISE NOTICE 'Roles en permissions: %', v_perm_roles;
