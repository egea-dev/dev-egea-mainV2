...
import { useProfile, useTasksByDate, useUpdateTaskState } from "@/hooks/use-supabase";
...
export default function WorkdayPage() {
  ...
  const { mutate: updateTaskState, isPending: isUpdatingState } = useUpdateTaskState();
  const [isProcessing, setIsProcessing] = useState(false);

  const handleStartTask = async (task: Task) => {
    if (isProcessing || isUpdatingState) return;
    ...
    try {
      ...
      // Update task to 'en fabricacion' (equivalent to In Progress)
      updateTaskState({ taskId: task.id, state: 'en fabricacion' });
        ...
        <section className="space-y-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2 self-start sm:self-auto">
              {activeTask && /* Simplificamos la condición para garantizar visibilidad */ (
                <>
                  {!['en fabricacion', 'terminado'].includes(activeTask.state || '') && (
                    <Button
                      variant="default"
                      size="sm"
                      className="bg-emerald-600 hover:bg-emerald-700 gap-2"
                      onClick={() => handleStartTask(activeTask)}
                      disabled={isProcessing || isUpdatingState || isSessionBusy}
                    >
                      <Clock className="h-4 w-4" />
                      Iniciar Tarea
                    </Button>
                  ...
                  {['en fabricacion', 'incidente', 'arreglo'].includes(activeTask.state || '') && (
                    <Button
                      variant="default"
                      size="sm"
                      className="bg-red-600 hover:bg-red-700 gap-2"
                      onClick={() => markTaskCompleted(activeTask.id)}
                      disabled={isProcessing || isUpdatingState}
                    >
