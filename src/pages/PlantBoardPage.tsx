...
export default function PlantBoardPage() {
    ...
    const fetchOrders = async () => {
        const { data: ordersData, error: ordersError } = await supabaseProductivity
            ...
            .select("*")
            .or("status.eq.PENDIENTE,status.eq.PAGADO,status.eq.EN_PROCESO,status.eq.CORTE,status.eq.EN_CORTE,status.eq.CONFECCION,status.eq.EN_CONFECCION,status.eq.TAPICERIA,status.eq.EN_TAPICERIA,status.eq.CONTROL_CALIDAD,status.eq.EN_CONTROL_CALIDAD")
            .order("priority", { ascending: false })
            .order("created_at", { ascending: false })
            .limit(100);

        if (ordersError) {
            console.error("Error fetching plant board data:", ordersError);
            return;
        }
        ...
        const transformed = (ordersData || []).map((order: any) => {
            ...
            // Priorizar: tabla relacional > JSON order > comercial
            const rawLines = relLines.length > 0 ? relLines : orderLines.length > 0 ? orderLines : commLines;
            // Deduplicar por material+dimensiones
            const grouped = rawLines.filter(Boolean).reduce((acc: Record<string, any>, line: any) => {
                const mat = line.material || line.fabric || 'Pieza';
                const key = `${mat}-${line.width || 0}x${line.height || 0}`;
                if (!acc[key]) acc[key] = { ...line, quantity: 0, material: mat };
            ...
            <div className="flex-1 px-8 py-4 space-y-3 overflow-y-auto custom-scrollbar">
                {orders.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-gray-700 space-y-4 opacity-50">
                        ...
                        <span className="text-2xl font-light uppercase tracking-widest">Sin trabajo en curso</span>
                        <p className="text-sm opacity-60">Los pedidos finalizados aparecen en el tablero de expediciones</p>
                    </div>
                ) : (
                    orders.map((order) => {
